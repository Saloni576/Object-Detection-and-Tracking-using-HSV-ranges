...
def crop_depth_frame(img: np.ndarray) -> np.ndarray:
    """Crop depth frame using the same cached box from color detection."""
    global global_crop_box
    if global_crop_box is None:
        # Detect from a sample color image once, if not done yet
        return img
    x_min, y_min, x_max, y_max = global_crop_box
    return img[y_min:y_max, x_min:x_max]

def find_matching_file(basename: str, directory: str, exts: List[str]):
    """Return path in directory with same basename and one of exts, or None."""
    for ext in exts:
...
def safe_read_mask(path: str, target_shape):
    """Read mask image and return binary mask (uint8 0/255) same size as target_shape (h,w)."""
...
def depth_to_mm(depth_raw: np.ndarray):
    """depth_raw: uint16 array. Return float32 mm array; invalid (0) -> 0"""
...
def draw_mask_overlay(img: np.ndarray, mask: np.ndarray, color=(0,255,255), alpha=0.4):
    """Overlay a semi-transparent mask color onto image. img is BGR."""
    overlay = img.copy()
...
def main():
    ...
    col_files = [f for f in os.listdir(col_dir) if f.lower().endswith((".png", ".jpg", ".jpeg"))]
    col_files.sort(key=natural_key)
    if len(col_files) == 0:
        print("No color frames found in", col_dir)
        return

    # CSV header
    csv_path = os.path.join(out_dir, "depths.csv")
    ...
    # Sample frame
    sample_color = cv2.imread(os.path.join(col_dir, col_files[0]))
    sample_color = crop_color_frame(sample_color)
    h, w = sample_color.shape[:2]
    combined_w = w * 2

    fourcc = cv2.VideoWriter_fourcc(*args.video_codec)
    combined_path = os.path.join(out_dir, "combined_side_by_side.mp4")
    depth_only_path = os.path.join(out_dir, "depth_only_masked.mp4")
    ...
    for idx, fname in enumerate(col_files, 1):
        ...
        color_path = os.path.join(col_dir, fname)
        depth_path = find_matching_file(basename, dep_dir, [".tiff", ".tif", ".png"])
        if depth_path is None:
            print(f"[WARN] no depth for {basename}; skipping")
            continue

        # --- Load and crop color & depth frames ---
        ...
        overlay_colors = {
            ...
            "green": (0,255,0),
            "gray": (255,0,0),
            "yellow_1": (0,255,255),   # bright yellow
            "yellow_2": (0,128,255),   # orange-yellow
            "gold": (0,215,255)        # gold-like color
        }

        for key, sub in mask_subs.items():
            mask_path = find_matching_file(basename, os.path.join(mask_dir, sub), [".png",".jpg",".jpeg",".tif",".tiff"])
